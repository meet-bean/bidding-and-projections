...
/** Determine which submenu/chip content type to use for a field. */
export type SubmenuType = 'exclusive' | 'dynamic' | 'operator-dynamic' | 'date';

export function getSubmenuType<T = unknown>(field: FilterFieldConfig<T>): SubmenuType {
  const type = field.type ?? 'select';
  if (type === 'date' || type === 'daterange') return 'date';
  if (type === 'multiselect') return 'operator-dynamic';
  if (type === 'select' && (field.searchable === true || field.maxSelections !== undefined))
    return 'dynamic';
  return 'exclusive';
}

/** Check if an operator is standalone (no value input needed). */
export function isStandaloneOperator(op: FilterOperator): boolean {
  if (op.needsValue !== undefined) return !op.needsValue;
  return op.value.toLowerCase().includes('empty');
}

/** Check if an operator value represents a standalone operator, looking it up in the operators list. */
export function isStandaloneOperatorValue(
  operatorValue: string,
  operators: FilterOperator[]
): boolean {
  const op = operators.find((o) => o.value === operatorValue);
  if (op) return isStandaloneOperator(op);
  return operatorValue.toLowerCase().includes('empty');
}
...
export const getOperatorsForField = <T = unknown>(
  ...
  if (fieldType === 'multiselect' || field.type === 'multiselect') {
    const multiselectOps = operators.multiselect;
    const selectOps = operators.select;
    return multiselectOps ?? selectOps ?? [];
  }

  const typeOps = operators[fieldType];
  const fallbackOps = operators.select;
  return typeOps ?? fallbackOps ?? [];
};
